...
void GameScene::RootDraw()
{
	...
	if (!m_PostProcessingMaterialIds.empty())
	{
		...
		for (size_t i = 0; i < m_PostProcessingMaterialIds.size(); i++)
		{
			...
			//4.4 Draw CURR_MAT (PREV_RT provides the Texture used in the PPEffect)
			//4.5 Change PREV_RT to TEMP_RT

			auto CURR_MAT = m_GameContext.pMaterialManager->GetMaterial_PP(m_PostProcessingMaterialIds[i]);
...
void GameScene::AddPostProcessingMaterial(UINT materialId_PP)
{
	...
	if (find(m_PostProcessingMaterialIds.begin(), m_PostProcessingMaterialIds.end(), materialId_PP) == end(m_PostProcessingMaterialIds))
	{
...
void GameScene::RemovePostProcessingMaterial(UINT materialId_PP)
{
	...
	auto material = std::find(m_PostProcessingMaterialIds.begin(), m_PostProcessingMaterialIds.end(), materialId_PP);
	m_PostProcessingMaterialIds.erase(material);
	//Hint: std::erase
}
