...
void GameObject::RootDraw_ShadowMap(const GameContext& gameContext)
{
	//Components ShadowMap
	for (BaseComponent* pComp : m_pComponents)
	{
		pComp->Draw_ShadowMap(gameContext);
	}

	//Root-Object ShadowMap
	for (GameObject* pChild : m_pChildren)
	{
		pChild->RootDraw_ShadowMap(gameContext);
	}
}
...
void GameObject::OnCollision(GameObject* colliderobject, GameObject* otherobject)
{
	if (m_OnColliderCallback)
		m_OnColliderCallback(colliderobject, otherobject);
}
...
void GameObject::SetOnColliderCallBack(ColliderCallback callback)
{
	m_OnColliderCallback = callback;
}
