...
void PhysxProxy::onContact(const PxContactPairHeader & pairHeader, const PxContactPair *pairs, PxU32 nbPairs)
{
	for (PxU32 i = 0; i < nbPairs; i++)
	{
		...
		auto colliderComponent = reinterpret_cast<BaseComponent*>(pairHeader.actors[i]->userData);
		auto otherComponent = reinterpret_cast<BaseComponent*>(pairHeader.actors[i + 1]->userData);


		if (colliderComponent != nullptr && otherComponent != nullptr)
		{
			GameObject* collider = colliderComponent->GetGameObject();
			GameObject* other = otherComponent->GetGameObject();

			if (pairs[i].events & PxPairFlag::eNOTIFY_TOUCH_FOUND)
				collider->OnCollision(collider, other);
