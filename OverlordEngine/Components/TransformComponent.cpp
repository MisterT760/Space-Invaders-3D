...
void TransformComponent::Scale(float x, float y, float z)
{
	...
	else
	{
		...
		auto type = GetGameObject()->GetComponent<ColliderComponent>()->GetShape()->getGeometry().getType();

		if (type == PxGeometryType::eCONVEXMESH)
		{
			auto mesh = GetGameObject()->GetComponent<ColliderComponent>()->GetShape()->getGeometry().convexMesh().convexMesh;
			PxConvexMeshGeometry rescaled(mesh, scale);
			GetGameObject()->GetComponent<ColliderComponent>()->GetShape()->setGeometry(rescaled);
		}
		else if (type == PxGeometryType::eTRIANGLEMESH)
		{
			auto mesh = GetGameObject()->GetComponent<ColliderComponent>()->GetShape()->getGeometry().triangleMesh().triangleMesh;
			PxTriangleMeshGeometry rescaled(mesh, scale);
			GetGameObject()->GetComponent<ColliderComponent>()->GetShape()->setGeometry(rescaled);
		}
