...
int DebugRenderer::m_NrOfLines = 0;
array<VertexPosCol, 100000> DebugRenderer::m_LineBuffer = array<VertexPosCol, 100000>();
...
void DebugRenderer::DrawLine(XMFLOAT3 start, XMFLOAT3 end, XMFLOAT4 color)
{
	if(!m_RendererEnabled)
		return;

	m_LineBuffer[m_NrOfLines] = VertexPosCol(start, color);
	m_LineBuffer[m_NrOfLines + 1] = VertexPosCol(end, color);
	m_NrOfLines += 2;
...
void DebugRenderer::DrawLine(XMFLOAT3 start, XMFLOAT4 colorStart, XMFLOAT3 end, XMFLOAT4 colorEnd)
{
	if(!m_RendererEnabled)
		return;

	m_LineBuffer[m_NrOfLines] = VertexPosCol(start, colorStart);
	m_LineBuffer[m_NrOfLines + 1] = VertexPosCol(end, colorEnd);
	m_NrOfLines += 2;
...
void DebugRenderer::DrawPhysX(PxScene* pScene)
{
	...
	for(UINT i = 0; i < pxDebugRenderer->getNbLines(); ++i)
	{
...
void DebugRenderer::Draw(const GameContext& gameContext)
{
	...
	if(size>0)
	{
		...
		memcpy(&(((VertexPosCol*)mappedResource.pData)[m_FixedBufferSize]), &m_LineBuffer[0], sizeof(VertexPosCol) * size);
