...
void SkinnedDiffuseMaterial::UpdateEffectVariables(const GameContext& gameContext, ModelComponent* pModelComponent)
{
	...
	if (m_pBoneTransformsVariable)
	{
		m_pBoneTransformsVariable->SetMatrixArray(reinterpret_cast<float*>(pModelComponent->GetAnimator()->GetBoneTransforms().data()), 0, pModelComponent->GetAnimator()->GetBoneTransforms().size());
	}
