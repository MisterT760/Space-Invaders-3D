...
float3 CalculateDiffuse(float3 normal, float2 texCoord)
{
	...
	if(gUseTextureDiffuse){
		...
		diffuseColor = gTextureDiffuse.Sample(gTextureSampler, texCoord) * diffuseLightValue * diffuseIntensity;
	}
...
float3 CalculateSpecularBlinn(float3 viewDirection, float3 normal, float2 texCoord)
{
	...
	float halfVector = -normalize(viewDirection + gLightDirection);
	float specularIntensity = dot(halfVector, normal);
	float specularStrength = max(pow(saturate(dot(halfVector, normal)), gShininess), 0);
	
	if(gUseTextureSpecularIntensity)
		specularIntensity = gTextureSpecularIntensity.Sample(gTextureSampler, texCoord) * specularIntensity;
	 
	specularColor = specularIntensity * gColorSpecular.rgb * specularStrength;
...
float3 CalculateSpecularPhong(float3 viewDirection, float3 normal, float2 texCoord)
{
	...
	float specularIntensity = dot(viewDirection, reflect(-gLightDirection, normal));
	float specularStrenght = max(pow(saturate(dot(viewDirection, reflect(-gLightDirection, normal))), gShininess), 0);
		
	if(gUseTextureSpecularIntensity)
		specularIntensity = gTextureSpecularIntensity.Sample(gTextureSampler, texCoord) * specularIntensity;
	
	specularColor = specularIntensity * gColorSpecular.rgb * specularStrenght;	
	return specularColor;
}
...
float3 CalculateFresnelFalloff(float3 normal, float3 viewDirection, float3 environmentColor)
{
	...
	if(gUseFresnelFalloff) {
		float fresnel = pow(saturate(1 - abs(dot(normal, viewDirection))), gFresnelPower) * gFresnelMultiplier;
		float fresnelMask = pow(abs(dot(float3(0, -1.f, 0), normal)), gFresnelHardness);
...
float3 CalculateEnvironment(float3 viewDirection, float3 normal)
{
	...
	if(gUseTextureEnvironment){
		environmentColor += gCubeEnvironment.Sample(gTextureSampler, reflect(viewDirection, normal)) * gReflectionStrength;
	}
