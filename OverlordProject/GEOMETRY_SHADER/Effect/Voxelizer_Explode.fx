...
//Explosion
float gExplosionForce = 10.0f;
float gGravity = 9.18f;

//Timers
float gTimer : TIME;
float gSlowMotionFactor = 1.0f;
float gMaxExplosionTime = 1.0f;
...
float3 Explode(float3 basePoint, float3 normal)
{
	float time = gTimer / gSlowMotionFactor;
	if(gTimer > gMaxExplosionTime)
	{
		time = gMaxExplosionTime;
	}
	
	float3 explosionPosition = basePoint;
		
	float gravityDisplacement = 0.5f * gGravity * pow(time, 2.0f);
	explosionPosition.y -= gravityDisplacement;
		
	float3 random = gNoiseTexture.SampleLevel(LinearSampler, normal, 1.0f);
	
	explosionPosition += gExplosionForce * normalize(normal) * random.x * time;
...
void Voxelize(VS_DATA vertices[3], inout TriangleStream<GS_DATA> triStream)
{
	...
	//Explode
	basePoint = Explode(basePoint, cubeNormal);
...
float4 MainPS(GS_DATA input) : SV_TARGET 
{
	...
	float color_a = diffuseColor.a - (gTimer / gMaxExplosionTime);
